...
public class TableRule implements Cloneable {
    ...
    public void parseTbSuffix() throws TableRule.ParseException {
        this.suffixManager.parseTbSuffix(this.dbIndexes);
    }

    public void init() throws TableRule.ParseException {
        if (this.dbIndexes == null) {
            ...
            for (int i = 0; i < this.dbIndexCount; i++) {
                ...
                this.dbIndexes[i] = new StringBuilder().append(this.dbIndexPrefix).append(suffix)
                    .toString();
            }
        }

        setDbIndexCount(this.dbIndexes.length);
        replaceWithParam(this.dbRules, this.dbRuleParames != null ? this.dbRuleParames
            : this.ruleParames);
        replaceWithParam(this.tbRules, this.tbRuleParames != null ? this.tbRuleParames
            : this.ruleParames);
        ...
        for (String param : params) {
            ...
            if (index == -1) {
                throw new IllegalArgumentException(new StringBuilder()
                    .append("使用名字化的占位符替换失败！请检查配置。 params:").append(Arrays.asList(params))
                    .toString());
            }
